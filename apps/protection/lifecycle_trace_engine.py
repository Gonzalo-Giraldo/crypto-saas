...
def trace_lifecycle_from_snapshot(
    snapshot: ProtectionEvidenceSnapshot,
) -> LifecycleDecisionTrace:
    evaluated_steps: list[LifecycleDecisionTraceStep] = []
    ...
    if snapshot.current_state == STATE_PROVISIONAL_ACTIVE:
        ...
        evaluated_steps.append(
            LifecycleDecisionTraceStep(
                evaluator_name="evaluate_equivalent_from_snapshot",
                decision=equivalent_decision,
            )
        )

        if equivalent_decision.allowed:
            final_decision = assert_allowed_transition(
                current_state=equivalent_decision.current_state,
                next_state=equivalent_decision.next_state,
            )

            return LifecycleDecisionTrace(
                current_state=snapshot.current_state,
                evaluated_steps=tuple(evaluated_steps),
        ...
        evaluated_steps.append(
            LifecycleDecisionTraceStep(
                evaluator_name="evaluate_replacement_from_snapshot",
    ...
    if snapshot.current_state == STATE_AUTHORITATIVE_REPLACEMENT_PENDING:
        ...
        evaluated_steps.append(
            LifecycleDecisionTraceStep(
                evaluator_name="evaluate_cleanup_start_from_snapshot",
                decision=cleanup_start_decision,
            )
        )
        ...
        return LifecycleDecisionTrace(
            current_state=snapshot.current_state,
            evaluated_steps=tuple(evaluated_steps),
            final_decision=cleanup_start_decision,
        )

    if snapshot.current_state == STATE_PROVISIONAL_CLEANUP_PENDING:
        cleanup_result_decision = evaluate_cleanup_result_from_snapshot(
            snapshot
        )
        evaluated_steps.append(
            LifecycleDecisionTraceStep(
                evaluator_name="evaluate_cleanup_result_from_snapshot",
                decision=cleanup_result_decision,
            )
        )
        ...
        return LifecycleDecisionTrace(
            current_state=snapshot.current_state,
            evaluated_steps=tuple(evaluated_steps),
            final_decision=cleanup_result_decision,
        )
    ...
    if snapshot.current_state == STATE_STALE_PROVISIONAL_PRESENT:
        ...
        evaluated_steps.append(
            LifecycleDecisionTraceStep(
                evaluator_name="evaluate_cleanup_retry_from_snapshot",
                decision=cleanup_retry_decision,
            )
        )
        ...
        return LifecycleDecisionTrace(
            current_state=snapshot.current_state,
            evaluated_steps=tuple(evaluated_steps),
            final_decision=cleanup_retry_decision,
        )
    ...
    if snapshot.current_state in {
        ...
        evaluated_steps.append(
            LifecycleDecisionTraceStep(
                evaluator_name="evaluate_trailing_from_snapshot",
        ...
        return LifecycleDecisionTrace(
            current_state=snapshot.current_state,
            evaluated_steps=tuple(evaluated_steps),
            final_decision=trailing_decision,
        )
