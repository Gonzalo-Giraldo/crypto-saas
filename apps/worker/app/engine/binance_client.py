...
def send_order_real(
    ...
    if response.status_code >= 400:
        raise RuntimeError(f"Binance order error {response.status_code}: {response.text}")

    body = response.json()
    if not isinstance(body, dict):
        raise RuntimeError("invalid_binance_order_response")
...
def query_order_status_by_order_id(
    ...
    if not order_id_norm:
        raise RuntimeError("order_id_required")
