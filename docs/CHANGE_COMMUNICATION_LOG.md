...
## db00a39 — binance runtime account-status gateway fallback test coverage
...
## 12a2cd3 — binance runtime send-test-order upstream error policy test coverage
...
## 8b317c5 — binance runtime send-test-order unsanitized fallback test coverage
...
## d531017 — binance runtime account-status entrypoint error mapping test coverage
- Added one isolated entrypoint test for Binance account-status runtime flow.
- Covered the path where internal account-status retrieval fails and runtime maps it to `HTTPException 502`.
- Confirmed the propagated detail is `Binance account status failed: gateway_upstream_error status=502`.
- No production logic changed.
- Validation evidence:
...
## 4384915 — binance runtime test-order entrypoint error mapping test coverage
- Added one isolated entrypoint test for Binance test-order runtime flow.
- Covered the path where internal send-test-order execution fails and runtime maps it to `HTTPException 502`.
- Confirmed the propagated detail is `Binance test order failed: gateway_upstream_error status=502`.
- No production logic changed.
- Validation evidence:
...
### Idempotent finalize policy clarification (auto-pick live)

During the idempotency/concurrency audit we confirmed that the auto-pick
live execution path may finalize a reserved idempotent intent twice
(500 then 200).

Decision:
Finalize must occur exactly once for a reserved intent.

Operational rule:
If an error-path finalize(status_code=500) happens, it is terminal and
the flow must not emit finalize(status_code=200) afterward for the same
reserved intent.

Implementation not yet applied.
This entry records the architectural decision before any code change.
